...
df <- read.csv("Nest Attendance/SHAP_values_nest_attendance.csv")
rawdf <- read.csv("Nest Attendance/RAW_SHAP_feature_values_nest_attendance.csv")
rawdf <- rawdf[, -1]
...
exclude_cols <- c("Unnamed: 0", "Expected_value", "Sum", 
                  "Prediction_probability", "nest_attendance", "frame")
...
ggsave(filename="good_shap_bees_nestatt_all_features.png", path = "graphs/", width = 6, height = 4.5)

#Beeswarm plot (top 6 features, colored by actual feature values)
  #made top 6 graphc bc thats what AMBER did
sv_importance(shp, kind = "bee", max_display = 6) + 
  labs(title = "Nest Attendance",x = "SHAP Score Change from Base Rate") + 
  theme_bw()
ggsave(filename="good_shap_bees_nestatt_top6.png", path = "graphs/", width = 6, height = 4.5)



#############
#LOLLIPOP PLOT BY FEATURE CATEGORIES
df_shap <- read.csv("Nest Attendance/SHAP_values_nest_attendance.csv")

#Get rid of SHAP meta data columns
exclude_cols <- c("...1", "Unnamed: 0", "Expected_value", "Sum", "Prediction_probability", "nest_attendance", "frame")
shap_cols <- setdiff(names(df_shap), exclude_cols)
...
ggplot(category_summary, aes(x = MeanSHAP, y = Category)) +
  ...
  labs(title = "Sum of SHAP Score by Feature Category",
       subtitle = "Nest Attendance",
    x = "Sum of Mean SHAP Scores", y = "Feature Category") +
  theme_bw(base_size = 14) + theme(legend.position = "none")
ggsave(filename="shap_lollipop_nestattend.png", path = "graphs/", width = 6, height = 4.5)
